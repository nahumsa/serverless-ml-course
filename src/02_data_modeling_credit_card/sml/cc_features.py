...
def expiry_days(trans_df: pd.DataFrame, credit_cards_df: pd.DataFrame) -> pd.DataFrame:
    ...
    card_expiry_df["expires"] = pd.to_datetime(
        card_expiry_df["expires"], format="%m/%y"
    )
...
def haversine_distance(
    long: float, lat: float, prev_long: float, prev_lat: float
) -> float:
...
def activity_level(trans_df: pd.DataFrame, lag: int) -> pd.DataFrame:
    ...
    trans_df[f"loc_delta_t_minus_{lag}"] = (
        trans_df.groupby("cc_num")
        .apply(
            lambda x: haversine_distance(
                x["longitude"],
                x["latitude"],
                x["longitude"].shift(-lag),
                x["latitude"].shift(-lag),
            )
        )
    ...
    trans_df[f"time_delta_t_minus_{lag}"] = (
        trans_df.groupby("cc_num")
    ...
    trans_df[f"time_delta_t_minus_{lag}"] = trans_df[f"time_delta_t_minus_{lag}"].map(
        lambda x: time_delta_to_days(x)
    )
    trans_df[f"time_delta_t_minus_{lag}"] = trans_df[
        f"time_delta_t_minus_{lag}"
    ].fillna(0)
    trans_df = trans_df[
        [
            "tid",
...
def aggregate_activity_by_hour(trans_df: pd.DataFrame, window_len) -> pd.DataFrame:
    ...
    cc_group = (
        trans_df[["cc_num", "amount", "datetime"]]
        .groupby("cc_num")
    ...
    cc_group = (
        trans_df[["cc_num", "loc_delta_t_minus_1", "datetime"]]
        .groupby("cc_num")
        .rolling(window_len, on="datetime")
        .mean()
    )
    ...
    window_aggs_df = window_aggs_df.merge(
        df_loc_delta_mavg, left_index=True, right_index=True
    )

    window_aggs_df = window_aggs_df.merge(
        trans_df[["cc_num", "datetime"]].sort_index(), left_index=True, right_index=True
    )
