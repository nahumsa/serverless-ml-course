        ...
        credit_cards.append(
            {
                ...
                "expires": faker.credit_card_expire(
                    start=delta_time_object, end="+5y", date_format="%m/%y"
                ),
            }
        )
    return credit_cards
    ...
    for category, category_perc_price in CATEGORY_PERC_PRICE.items():
        ...
        for _ in range(n):
            ...
            categories.append(
                {
                    "category": category,
                    "amount": random.choice(amounts[min_price_i:max_price_i]),
                }
            )
...
def generate_transaction_id(
    timestamp: str, credit_card_number: str, transaction_amount: float
) -> str:
    """."""
...
def generate_transactions(
    credit_card_numbers: list, timestamps: list, categories: list
) -> list:
    """."""
    transactions = []
    for timestamp, category in zip(timestamps, categories):
        ...
        transaction_id = generate_transaction_id(
            timestamp, credit_card_number, category["amount"]
        )
        transactions.append(
            {
                "tid": transaction_id,
...
def generate_susceptible_cards(credit_cards: list) -> list:
    ...
    for percentage, span in SUSCEPTIBLE_CARDS_DISTRIBUTION_BY_AGE.items():
        n = int(
            TOTAL_UNIQUE_CASH_WITHDRAWALS * percentage
        )  ## TODO: here total expected fraud
        start, end = span
        for _ in range(n):
...
def generate_normal_atm_withdrawals(
    cash_amounts: list, susceptible_cards: list
) -> list:
    """."""
    normal_atm_withdrawals = []
    atm_transactions = len(cash_amounts)
    cash_withdrawal_cards = random.sample(
        susceptible_cards,
        CASH_WITHRAWAL_CARDS_TOTAL
    ...
    while atm_count < atm_transactions:
        for card in cash_withdrawal_cards:
            for ATM_WITHRAWAL_SEQ in ATM_WITHRAWAL_SEQ_LENGTH:
                ...
                atm_tr = generate_atm_withdrawal(
                    credit_card_number=card["cc_num"],
                    cash_amounts=cash_amounts,
                    length=ATM_WITHRAWAL_SEQ,
                    delta=delta,
                    radius=NORMAL_ATM_RADIUS,
                )
    ...
    for key, chain in chains.items():
        ...
        for i, idx in enumerate(chain):
            ...
            original_transaction["category"] = [
                category
                for category, category_perc_price in CATEGORY_PERC_PRICE.items()
                if int(inject_amounts[i])
                in range(int(category_perc_price[1]), int(category_perc_price[2]))
            ][0]
            original_transaction["tid"] = generate_transaction_id(
                inject_timestamp, cc_num, amount
            )
...
def generate_fraudulent_atm_tr_indxs(normal_atm_withdrawals: list) -> list:
    ...
    return random.sample(
        [i for i in range(0, len(normal_atm_withdrawals))],
        int(FRAUD_RATIO * len(normal_atm_withdrawals)),
    )


def update_normal_atm_withdrawals(
    fraudulent_atm_tr_indxs: list, normal_atm_withdrawals: list, cash_amounts: list
):
    """."""
    for fraudulent_atm_tr_indx in fraudulent_atm_tr_indxs:
        ...
        fraudulent_atm_tr = generate_atm_withdrawal(
            credit_card_number=pre_fraudulent_atm_tr["cc_num"],
            cash_amounts=cash_amounts,
            length=1,
            delta=delta,
            radius=None,
        ...
        while fraudulent_atm_location[3] == "US":
            fraudulent_atm_location = faker.location_on_land()
        fraudulent_atm_tr["datetime"] = (
            datetime.datetime.strptime(pre_fraudulent_atm_tr["datetime"], DATE_FORMAT)
...
def transactions_as_dataframe(
    transactions: list, normal_atm_withdrawals: list
) -> pd.DataFrame:
    """."""
...
def create_transactions_as_df(credit_cards: list) -> pd.DataFrame:
    ...
    normal_atm_withdrawals = generate_normal_atm_withdrawals(
        cash_amounts, susceptible_cards
    )
    update_transactions(transactions, chains)

    fraudulent_atm_tr_indxs = generate_fraudulent_atm_tr_indxs(normal_atm_withdrawals)
    update_normal_atm_withdrawals(
        fraudulent_atm_tr_indxs, normal_atm_withdrawals, cash_amounts
    )
